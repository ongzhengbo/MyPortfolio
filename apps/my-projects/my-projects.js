...
class MyProjectsApp {
          ...
          <div class="steam-store-header">
            ...
            <div class="steam-featured-section">
              ...
              <div class="steam-carousel-nav">
                ${featured.map((_, i) => `<button class="steam-carousel-dot ${i === 0 ? 'active' : ''}" data-index="${i}" onclick="window.PortfolioApps['My Projects'].setSteamHero(${i})"></button>`).join('')}
              </div>
            </div>
          </div>
  ...
  openSteamDetail(projectId) {
    ...
    if (detailPage) {
      ...
      thumbsContainer.innerHTML = project.screenshots.map((s, i) => `<img src="${s}" alt="screenshot" class="${i === 0 ? 'active' : ''}" onclick="window.PortfolioApps['My Projects'].setSteamDetailMedia('${s}')">`).join('');
    }
  }
